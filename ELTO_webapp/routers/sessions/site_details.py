...
from routers.warning_utils import WARNING_DETAILS, get_warning_detail, get_warning_details, parse_warning_codes
...
async def get_sessions_site_details(
    ...
    where_clause, params = _build_conditions(
        sites, date_debut, date_fin,
        energy_mode=energy_mode, energy_min=energy_min, energy_max=energy_max,
        temp_column=temp_column, temp_mode=temp_mode, temp_min=temp_min, temp_max=temp_max,
    ...
    warning_series = df["warning"] if "warning" in df.columns else pd.Series([0] * len(df), index=df.index)
    df["warning_count"] = warning_series.apply(lambda x: len(parse_warning_codes(x)))
    df["is_warning"] = df["warning_count"].gt(0)
    ...
    if "warning" in err_rows.columns:
        err_rows["warning_detail"] = err_rows["warning"].apply(get_warning_detail)
        err_rows["warning_details_list"] = err_rows["warning"].apply(get_warning_details)
        err_rows["warning_display"] = err_rows["warning"].apply(
            lambda x: ", ".join(str(code) for code in parse_warning_codes(x))
        )
    ...
    if "warning" in ok_rows.columns:
        ok_rows["warning_detail"] = ok_rows["warning"].apply(get_warning_detail)
        ok_rows["warning_details_list"] = ok_rows["warning"].apply(get_warning_details)
        ok_rows["warning_display"] = ok_rows["warning"].apply(
            lambda x: ", ".join(str(code) for code in parse_warning_codes(x))
        )
