...
from routers.warning_utils import WARNING_DETAILS, get_warning_details, parse_warning_codes
...
async def search_mac(
    ...
    df["warning"] = pd.to_numeric(df.get("warning", 0), errors="coerce").fillna(0).astype(int)
    df["is_warning"] = df["warning"].gt(0)
...
async def search_by_codes(
    ...
    df["warning_codes_list"] = warning_series.apply(parse_warning_codes)
    df["warning_has_codes"] = df["warning_codes_list"].apply(lambda codes: len(codes) > 0)
    df["warning_details_list"] = warning_series.apply(get_warning_details)
