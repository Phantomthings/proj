...
async def get_overview(
        ...
        for site_name, df_site in df_defauts.groupby("site"):
            ...
            for label, pattern in equip_patterns:
                if pattern:
                    ...
                    # Autres = tout ce qui n'a pas été traité
    ...
    # ============================================================
    # 2. CÂBLES DE RECHARGE COUPÉS (PC08)
    # ============================================================
    sql_cables = """
        SELECT site, eqp, date_debut
        FROM kpi_defauts_log
        WHERE date_fin IS NULL
          AND defaut LIKE 'PC08%'
        ORDER BY site, date_debut
    """
    df_cables = query_df(sql_cables)

    cables_par_site = []
    if not df_cables.empty:
        if site_list:
            df_cables = df_cables[df_cables["site"].isin(site_list)]
        if not df_cables.empty:
            df_cables["date_debut"] = pd.to_datetime(df_cables["date_debut"], errors="coerce")
            now = pd.Timestamp.now()
            df_cables["depuis_jours"] = (now - df_cables["date_debut"]).dt.days
            df_cables["is_recent"] = (now - df_cables["date_debut"]) < pd.Timedelta(hours=24)
            for site_name, grp in df_cables.groupby("site"):
                cables_par_site.append({
                    "site": site_name,
                    "has_recent": bool(grp["is_recent"].any()),
                    "equipements": [
                        {
                            "eqp": row["eqp"],
                            "depuis_jours": int(row["depuis_jours"]),
    ...
    # ============================================================
    # 3. TRANSACTIONS SUSPECTES (<1 kWh)
    # ============================================================
    ...
    # ============================================================
    # 6. STATS SESSIONS (Taux réussite / Échecs)
    # Applique les filtres error_types et moments sur is_ok
    # ============================================================
    ...
    if not df_sessions.empty:
        # is_ok de base : state == 0
        df_sessions["is_ok"] = pd.to_numeric(df_sessions["state"], errors="coerce").fillna(0).astype(int).eq(0)
        
        # Appliquer les filtres error_types et moments sur les échecs
        # mask_nok = sessions en erreur
        # mask_type = type_erreur dans la liste sélectionnée
        # mask_moment = moment dans la liste sélectionnée
        # is_ok_filt = True SAUF si (not is_ok AND mask_type AND mask_moment)
    ...
    return templates.TemplateResponse(
        "partials/tab_overview.html",
        {
            ...
            "pdc_only": pdc_only,
            # Câbles coupés (PC08)
            "cables_par_site": cables_par_site,
            "nb_cables_recent": sum(1 for s in cables_par_site if s["has_recent"]),
            # Suspicious
